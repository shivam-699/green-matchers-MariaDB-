    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-12">
        <div className="glass-effect rounded-2xl p-12">
          ...
          <h2 className="text-3xl font-bold text-white mb-4">{t('auth.required') || 'Authentication Required'}</h2>
          <p className="text-slate-300 mb-8 text-lg">{t('auth.loginToAccess') || 'Please login to access advanced job search features'}</p>
          <Link
            to="/login"
  ...
  return (
    <div className="max-w-7xl mx-auto w-full">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold text-white mb-4">{t('job.search') || 'Find Green Jobs'}</h1>
        ...
        <div className="lg:col-span-1">
          <div className="glass-effect rounded-2xl p-6 sticky top-4">
            <h2 className="text-xl font-bold text-white mb-6 flex items-center gap-2">
              <span>🎯</span> {t('common.filters') || 'Search Filters'}
            </h2>
          ...
          <div className="glass-effect rounded-2xl p-6">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-xl font-bold text-white flex items-center gap-2">
                <span>💼</span>
                {searchPerformed 
                  ? `${t('job.searchResults') || 'Search Results'} (${jobs.length})` 
                  : `${t('job.featuredJobs') || 'Featured Jobs'} (${jobs.length})`
                }
              </h2>
              <div className="text-sm text-slate-400">
                {t('job.sortedBy') || 'Sorted by'}: <span className="text-blue-400">{t('job.bestMatch') || 'Best Match'}</span>
              </div>
            </div>
            ...
            {jobs.length === 0 ? (
              <div className="text-center py-12">
                ...
                <h3 className="text-xl font-bold text-white mb-2">{t('job.noJobs') || 'No jobs found'}</h3>
                <p className="text-slate-400">{t('job.tryAdjustingFilters') || 'Try adjusting your filters'}</p>
              </div>
            ) : (
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 text-sm text-slate-300 flex-wrap">
                          ...
                          <span className="flex items-center gap-1">
                            ⏰ {job.postedDays}d {t('common.ago') || 'ago'}
                          </span>
                        </div>
                      </div>
